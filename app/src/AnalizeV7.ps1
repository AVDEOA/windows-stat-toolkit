...
function Get-SecurityAuditData {
    ...
    if ($successfulRemoteAccess.Count -gt 0) {
        [void]$findings.Add([pscustomobject]@{
            ...
            severity = 'medium'
            title = 'Обнаружены реальные RDP-входы с корреляцией 1149 + 4624(LogonType 10)'
            summary = ('Найдено {0} скоррелированных реальных RDP-сессий за выбранный период.' -f $successfulRemoteAccess.Count)
            evidence = @($successfulRemoteAccess | Select-Object -First 10)
        })
        Add-UniqueText -Target $recommendedActions -Text 'Проверить, все ли реальные RDP-сессии ожидаемы, особенно вне рабочего времени и с внешних IP.'
    }
...
$analysis.purpose = 'Forensic + inventory + configuration audit + security audit JSON v7 для чтения и анализа нейросетью по состоянию юнита, ошибкам, конфигурации, удаленному доступу, шуму security-событий и вероятным причинам.'
...
Write-Host ''
Write-Host ('Файл анализа V7 создан: {0}' -f $outputPath)
