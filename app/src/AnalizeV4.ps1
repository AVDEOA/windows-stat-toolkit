...
function Get-DeadlineContext {
    ...
    $existingPaths = @(
        foreach ($path in @($candidatePaths)) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($trimmed) -and (Test-Path -Path $trimmed)) {
                [pscustomobject]@{
                    path = $trimmed
                    type = $(if (Test-Path -Path $trimmed -PathType Container) { 'directory' } else { 'file' })
                }
            }
        }
    )
...
function Get-ReliabilityMonitorData {
    $metrics = @(
        Ensure-Array (Get-CimSafe -ClassName 'Win32_ReliabilityStabilityMetrics') |
            ...
            ForEach-Object {
                [pscustomobject]@{
                    time_generated = (Invoke-Safely -Fallback $null -ScriptBlock { [Management.ManagementDateTimeConverter]::ToDateTime($_.TimeGenerated).ToString('yyyy-MM-dd HH:mm:ss') })
...
$outputPath = Join-Path -Path $outputRoot -ChildPath 'AnalizeV4.txt'
$now = Get-Date
...
$analysis.purpose = 'Forensic + inventory + configuration audit JSON v4 для чтения и анализа нейросетью по состоянию юнита, ошибкам, конфигурации и вероятным причинам.'
...
Write-Host ''
Write-Host ('Файл анализа V4 создан: {0}' -f $outputPath)
