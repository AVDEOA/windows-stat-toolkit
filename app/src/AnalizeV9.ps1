...
function Get-SecurityExplanation {
    ...
    switch ([string]$EventId) {
        '4624' {
            if ($RemoteAccessKind -eq 'real_remote_logon') { return 'Зафиксирован успешный удалённый вход с LogonType 10 и не-локальным источником.' }
            if ($RemoteAccessKind -eq 'local_or_internal_logon_type_10') { return 'LogonType 10 зафиксирован, но источник выглядит локальным или внутренним; это не стоит автоматически трактовать как внешний удалённый доступ.' }
            'Зафиксирован успешный вход в систему.'
        }
        ...
        '1149' {
            if ($SourceScope -eq 'local') { return 'Успешная RDP-аутентификация, но источник помечен как LOCAL; это может быть внутренний session-manager шум, а не внешний удалённый вход.' }
            'Успешная аутентификация через Remote Desktop Services.'
        }
        default {
    ...
    switch ([string]$EventId) {
        ...
        default {
            if ($RemoteAccessKind -eq 'local_or_internal_logon_type_10') {
                return 'Не считать событие полноценным внешним remote access без дополнительной корреляции по 1149, 4624 и источнику.'
            }
            if (-not [string]::IsNullOrWhiteSpace($SourceIp)) {
...
function Get-SecurityServiceInstallContext {
    ...
    [pscustomobject]@{
        ...
        expected_change_candidate = $(if (($signatureStatus -eq 'Valid') -and (-not [string]::IsNullOrWhiteSpace($knownVendor) -or $relatedSoftware.Count -gt 0)) { $true } else { $false })
    }
}
...
function Get-SecuritySignalClassification {
    ...
    if ($Record.category -eq 'firewall' -and $Record.full_message -match '(?i)(veeam|windefend|microsoft defender|msiexec\.exe|startsexperiencesapp|securityhealth|microsoft corporation)') {
        return 'maintenance_expected'
    }
    ...
    @(
        foreach ($authEvent in $rdpAuthEvents) {
            ...
            $lifecycleBySource = @(
                $lifecycleEvents |
                    Where-Object {
                        ($_.source_ip -eq $authEvent.source_ip -and -not [string]::IsNullOrWhiteSpace($authEvent.source_ip)) -or
                        ($_.username -eq $authEvent.username -and -not [string]::IsNullOrWhiteSpace($authEvent.username))
                    } |
                    Where-Object {
                        $candidateTime = Invoke-Safely -Fallback $null -ScriptBlock { [datetime]::ParseExact($_.timestamp, 'yyyy-MM-dd HH:mm:ss', $null) }
                        $authTime -and $candidateTime -and ([math]::Abs(($candidateTime - $authTime).TotalMinutes) -le 60)
                    } |
                    Sort-Object timestamp
            )
            ...
            [pscustomobject]@{
                ...
                classification = $(if ($matchedLogon.Count -gt 0) { 'correlated_real_rdp_session' } elseif ($authEvent.remote_access_scope -eq 'remote' -and $lifecycleBySource.Count -gt 0) { 'probable_remote_rdp_session' } elseif ($authEvent.remote_access_scope -eq 'remote') { 'rdp_auth_only_remote_source' } else { 'local_or_internal_rdp_noise' })
                auth_event = $authEvent
                logon_event = $(if ($matchedLogon.Count -gt 0) { $matchedLogon[0] } else { $null })
                disconnect_event = $disconnectEvent
                lifecycle_events = @($lifecycleBySource | Select-Object -First 8)
            }
        }
    )
}
...
function Get-SecurityAuditData {
    ...
    $successfulRemoteAccess = @($rdpSessionCorrelation | Where-Object { $_.classification -in @('correlated_real_rdp_session', 'probable_remote_rdp_session') })
    $probableRemoteAccess = @($rdpSessionCorrelation | Where-Object { $_.classification -eq 'probable_remote_rdp_session' })
    ...
    foreach ($group in $failedGroups) {
        ...
        [void]$findings.Add([pscustomobject]@{
            ...
            severity = 'high'
            title = 'Есть серия неуспешных удалённых входов, похожая на brute-force candidate'
            summary = ('За короткое окно найдено {0} неуспешных удалённых входов для пользователя {1} с IP {2}.' -f $group.Count, $(if ($sample.username) { $sample.username } else { 'unknown' }), $(if ($sample.source_ip) { $sample.source_ip } else { 'unknown' }))
    ...
    if ($successfulRemoteAccess.Count -gt 0) {
        [void]$findings.Add([pscustomobject]@{
            ...
            severity = 'medium'
            title = 'Observed remote RDP session activity'
            summary = ('Found {0} correlated or probable remote RDP sessions in the selected period.' -f $successfulRemoteAccess.Count)
            evidence = @($successfulRemoteAccess | Select-Object -First 10)
        })
        Add-UniqueText -Target $recommendedActions -Text 'Review correlated and probable remote RDP sessions, especially off-hours or external.'
    }
    ...
    if ($blockedRemoteAccess.Count -gt 0) {
        [void]$findings.Add([pscustomobject]@{
            ...
            summary = ('Найдено {0} blocked events 5157 на RDP/WinRM/SSH-порты от не-локальных источников.' -f $blockedRemoteAccess.Count)
    ...
    if ($highRiskConfigEvents.Count -gt 0) {
        [void]$findings.Add([pscustomobject]@{
            ...
            severity = 'high'
            title = 'Есть высокоприоритетные изменения security-конфигурации или установка служб'
            summary = ('Найдено {0} событий по audit policy / log cleared / service install без признаков ожидаемого vendor-backed контекста.' -f $highRiskConfigEvents.Count)
    ...
    if ($offHoursRemoteAccess.Count -gt 0) {
        [void]$findings.Add([pscustomobject]@{
            ...
            severity = 'medium'
            title = 'Есть реальные удалённые RDP-сессии в нерабочее время'
            summary = ('Найдено {0} скоррелированных удалённых RDP-сессий в диапазоне 21:00-06:00.' -f $offHoursRemoteAccess.Count)
    ...
    if (($failedLogonsAll.Count -eq 0) -or (@($firewallAndPortActivityAll | Where-Object { $_.action -eq 'blocked' }).Count -eq 0)) {
        [void]$findings.Add([pscustomobject]@{
            ...
            severity = 'medium'
            title = 'Покрытие security-аудита ограничивает уверенность по failed logons и blocked connections'
            summary = 'Часть security-выводов нельзя трактовать как исчерпывающий факт отсутствия событий без дополнительной проверки audit policy и firewall/WFP auditing.'
            evidence = @([pscustomobject]@{
                audit_policy = $auditCoverage
                coverage_notes = @($coverageNotes)
            })
        })
        Add-UniqueText -Target $recommendedActions -Text 'Проверить audit policy для Logon/Failure и Filtering Platform Connection/Packet Drop, если нужна высокая уверенность по отсутствующим failed/block events.'
    }
    ...
    $summary = [ordered]@{
        ...
        vendor_backed_expected_change_count = @($securityConfigurationAudit | Where-Object { $_.expected_change_candidate -eq $true }).Count
        ...
        probable_remote_session_count = $probableRemoteAccess.Count
    ...
    [pscustomobject]@{
        ...
        probable_remote_access = @($probableRemoteAccess | Sort-Object auth_timestamp -Descending | Select-Object -First 80)
        ...
        security_noise_filters = [ordered]@{
            classification_rules = @(
                'Defender health events 1150/1151/2010 классифицируются как background_noise.',
                'Defender 5007 по умолчанию классифицируется как maintenance_expected.',
                'Firewall duplicate-rule spam и local session-manager RDP lifecycle классифицируются как background_noise.',
                '7045/4697 со signed vendor-backed контекстом помечаются как maintenance_expected.'
            )
...
function Get-DateFromLooseText {
    param([object]$Value)
    ...
    if ($text -match '^\d{8}$') {
        return Invoke-Safely -Fallback $null -ScriptBlock { [datetime]::ParseExact($text, 'yyyyMMdd', $null) }
    }

    Invoke-Safely -Fallback $null -ScriptBlock { [datetime]$Value }
}

function Get-WeeklyChangeSummary {
    param(
        [datetime]$StartTime,
        [object[]]$InstalledSoftware,
        [object[]]$WindowsUpdates,
        [object[]]$DriverUpdateHistory,
    ...
    $recentSoftware = @(
        $InstalledSoftware |
            Where-Object {
                $installed = Get-DateFromLooseText -Value $_.install_date
                $installed -and $installed -ge $StartTime
            } |
            Sort-Object install_date -Descending |
            Select-Object -First 30
    )

    $serviceInstalls = @(
        $SecurityAudit.security_configuration_audit |
            Where-Object { $_.event_id -in @('4697', '7045') } |
            Sort-Object timestamp -Descending |
            Select-Object -First 30
    )

    $firewallRuleChanges = @(
        $SecurityAudit.firewall_and_port_activity |
            Where-Object {
                $_.full_message -match '(?i)(rule|profile|firewall)' -or $_.event_id -in @('2084', '2004', '2005', '2006', '2033', '2034')
            } |
            Sort-Object timestamp -Descending |
    ...
    $newRemoteSources = @(
        $SecurityAudit.rdp_authentication |
            ForEach-Object { $_.source_ip } |
            Where-Object { -not [string]::IsNullOrWhiteSpace($_) -and $_ -notmatch '^(?i)(LOCAL|127\.0\.0\.1|::1|-)$' } |
            Sort-Object -Unique |
            Select-Object -First 20
    )

    [ordered]@{
        driver_changes = [ordered]@{
            count = $DriverUpdateHistory.Count
            items = @($DriverUpdateHistory | Select-Object -First 40)
        }
        windows_updates = [ordered]@{
            count = $WindowsUpdates.Count
            items = @($WindowsUpdates | Select-Object -First 20)
        }
        software_installed = [ordered]@{
            count = $recentSoftware.Count
            items = @($recentSoftware)
        }
        service_installs = [ordered]@{
            count = $serviceInstalls.Count
            items = @($serviceInstalls)
        }
        firewall_rule_changes = [ordered]@{
            count = $firewallRuleChanges.Count
            items = @($firewallRuleChanges)
        }
        remote_access_sources = [ordered]@{
            count = $newRemoteSources.Count
            items = @($newRemoteSources)
        }
    }
}
...
$weeklyChangeSummary = Get-WeeklyChangeSummary -StartTime $selection.StartTime -InstalledSoftware $installedSoftware -WindowsUpdates $windowsUpdateInventory -DriverUpdateHistory $driverUpdateHistory -SecurityAudit $securityAudit
...
$aiSummary += ('Security expected changes: vendor_backed={0}; probable_remote_sessions={1}.' -f $securitySummary.vendor_backed_expected_change_count, $securitySummary.probable_remote_session_count)
...
$aiSummary += ('Weekly changes: drivers={0}, updates={1}, service_installs={2}, firewall_rule_changes={3}, remote_sources={4}.' -f $weeklyChangeSummary.driver_changes.count, $weeklyChangeSummary.windows_updates.count, $weeklyChangeSummary.service_installs.count, $weeklyChangeSummary.firewall_rule_changes.count, $weeklyChangeSummary.remote_access_sources.count)
...
$diagnostics.weekly_change_summary = $weeklyChangeSummary
...
$inventory.security_high_priority = @($securityAudit.high_priority_security_alerts)
...
$analysis.purpose = 'Forensic + inventory + configuration audit + security audit JSON v9 for weekly and period-based operational analysis.'
...
$analysis.what_changed_this_week = $weeklyChangeSummary
...
$analysis.security_audit = [ordered]@{
    ...
    probable_remote_access = @($securityAudit.probable_remote_access)
    ...
    what_changed_this_week = $weeklyChangeSummary
}
...
Write-Host ''
Write-Host ('Файл анализа V9 создан: {0}' -f $outputPath)
