...
- Remote execution still uses `ssh.exe` + remote `powershell.exe`, but that dependency is isolated behind `IRemoteDiagnosticsTransport`.
...
- Host management, report actions, and monitoring live in the compiled desktop app.
...
1. Replace the current SSH + remote PowerShell collector with a more native C# collection pipeline where feasible.
2. Move long-running collection and monitor work off the UI thread into background jobs/services.
...
5. Add stronger tests around parsing, categorization, bundle trimming, and report composition.
